...
def validateTimeSlicePostRequest(request, runs):
    ...
    try:
        ...
        if subsystemName in run.subsystems.keys():
            ...
            error.setdefault("subsystem", []).append(
                "Subsystem name {0} is not available in {1}!".format(subsystemName, run.prettyName))
            # Invalidate and we cannot continue
            return (error, None, None, None, None, None, None, None, None)
        ...
        if maxTime > subsystem.runLength:
            error.setdefault("maxTime", []).append(
                "Max time of {0} greater than the run length of {1}".format(maxTime, subsystem.runLength))
        if minTime > maxTime:
        ...
        if hotChannelThreshold < 0 or hotChannelThreshold > 1000:
            error.setdefault("hotChannelThreshold", []).append(
                "Hot channel threshold {0} is outside the possible range of 0-1000!".format(hotChannelThreshold))
...
def validateRunPage(runDir, subsystemName, requestedFileType, runs):
    ...
    try:
        ...
        if runDir in runs.keys():
            ...
            error.setdefault("Run Dir", []).append(
                "{0} is not a valid run dir! Please select a different run!".format(runDir))
            # Invalidate and we cannot continue
        ...
        if subsystemName in run.subsystems.keys():
            ...
            error.setdefault("Subsystem", []).append(
                "{0} is not a valid subsystem in {1}!".format(subsystemName, run.prettyName))
            # Invalidate and we cannot continue
            return (error, None, None, None, None, None, None, None, None, None)

        # Validate requested file type
        if requestedFileType not in ["runPage", "rootFiles"]:
            error.setdefault("Request Error", []).append(
                "Requested: {0}. Must request either runPage or rootFiles!".format(requestedFileType))
    ...
    if error == {}:
        return (
            error, run, subsystem, requestedFileType, jsRoot, ajaxRequest, requestedHistGroup, requestedHist,
            timeSliceKey,
            timeSlice)
    else:
        return (error, None, None, None, None, None, None, None, None, None)
...
def validateHistGroupAndHistName(histGroup, histName, subsystem, run, error):
    ...
    if histGroup:
        ...
        for i, group in enumerate(subsystem.histGroups):
            ...
            if histGroup == group.selectionPattern:
                ...
                if histName and histName not in subsystem.histGroups[i].histList:
                    error.setdefault("histName", []).append(
                        "histName {0} is not available in histGroup {1} in {2}".format(histName, histGroup,
                                                                                       run.prettyName))
        ...
        if not foundHistGroup:
            error.setdefault("histGroup", []).append(
                "histGroup {0} is not available in {1}".format(histGroup, run.prettyName))
    else:
        if histName and histName not in subsystem.hists.keys():
            error.setdefault("histName", []).append(
                "histName {0} is not available in {1}".format(histName, run.prettyName))
...
def retrieveAndValidateTimeSlice(subsystem, error):
    ...
    if timeSliceKey:
        ...
        if timeSliceKey == "fullProcessing":
            ...
            error.setdefault("timeSliceKey", []).append(
                "{0} is not a valid time slice key! Valid time slices include {1}."
                " Please select a different time slice!".format(timeSliceKey, subsystem.timeSlices))
            timeSlice = None
    else:
